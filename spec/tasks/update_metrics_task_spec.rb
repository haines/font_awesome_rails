...
describe UpdateMetricsTask do
  ...
  describe "#font_class" do
    it "returns the source code for FontAwesome::Font" do
      task.expects(:open_file).with("lib/font_awesome/font.rb").yields(mock{expects(:read)}).returns("source code")
  ...
  describe "#run" do
    it "writes the updated source code" do
      ...
      task.expects(:open_file).with("lib/font_awesome/font.rb", "w").yields(mock{expects(:write).with("updated source code")})
      task.run
    end
  end
end
